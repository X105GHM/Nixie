...
float NtcThermistor::readTemperatureC() const noexcept
{
    ...
    if (result != ESP_OK)
    {
        Logger::log(LoggerType::SENSOR, "ADC read failed: %s", esp_err_to_name(result));
    ...
    if (v <= 0.0f || v >= 3.3f)
    {
        Logger::log(LoggerType::SENSOR, "ADC voltage out of NTC range: %d mV", measurement.millivolts);
        return std::numeric_limits<float>::quiet_NaN();
    }
