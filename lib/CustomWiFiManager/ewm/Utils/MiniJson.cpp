...
namespace
{
    ...
    {
        if (i >= (int)b.length() || b[i] != '"') return false;
        ++i;
        out = "";
        ...
        while (i < (int)b.length())
        {
            ...
            if (c == '\\')
            {
                if (i >= (int)b.length()) return false;
                char e = b[i++];
                switch (e)
                {
                    case '"': out += '"'; break;
                    ...
                    case 'u':
                    {
                        if (i + 4 > (int)b.length()) return false;
                        uint16_t v = 0;
                        for (int k=0;k<4;k++)
                        {
                            char h = b[i++];
...
namespace ewm::utils
{
    ...
    bool json_get_int(const String& b, const char* key, int& out)
    {
        ...
        bool neg = false;
        if (p < (int)b.length() && b[p] == '-') { neg = true; ++p; }
    ...
    bool json_get_order_array(const String& b, std::vector<String>& order)
    {
        ...
        {
            json_skip_ws(b, p);
            if (p < (int)b.length() && b[p] == ']') return true;

            String s;
            if (!json_read_string(b, p, s)) return false;
            order.push_back(s);

            json_skip_ws(b, p);
            if (p < (int)b.length() && b[p] == ',') { ++p; continue; }
            if (p < (int)b.length() && b[p] == ']') return true;
        }
        return false;
    }
